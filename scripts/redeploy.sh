...
if [ "$(docker inspect caddy --format '{{.State.Running}}' 2>/dev/null)" != "true" ]; then
...
SITE="$(docker inspect caddy --format '{{range .Config.Env}}{{println .}}{{end}}' 2>/dev/null \
  | sed -n 's/^NCE_SITE_ADDR=//p')"
