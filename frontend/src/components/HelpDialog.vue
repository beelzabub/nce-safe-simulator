...
const CATEGORY_DEFS = [
  ...
  { name: 'Seed Data',      description: 'Generate and simulate realistic test conditions',         tools: ['generate-issues', 'generate-epic-blocks', 'generate-issue-blocks', 'generate-roam-risks', 'generate-risk-reasons', 'close-percent', 'simulate-pi-progress', 'set-epic-states', 'orphan-epics', 'orphan-issues'] },
...
const REPORTS = [
  ...
  { key: 'epic-lifecycle',        label: 'Epic Lifecycle',             description: 'Lifecycle stage distribution across the portfolio — how many epics are in funnel, backlog, implementing, and done at each level. Also flags "stuck" items: epics that have sat in funnel, analyzing, or backlog beyond their configured age thresholds (defaults 90 / 30 / 60 days, set in Config → Reports; age measured from created_at).' },
