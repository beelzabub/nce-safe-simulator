...
TOOLS = [
    ...
    },
    {
        ...
        "params": [
            ...
            {"name": "group",              "prompt": "Target group (import into)",                                              "type": str,  "widget": "group", "optional": True},
            {"name": "create_missing",     "prompt": "Create the target root group if it doesn't exist (subgroups are NOT created)",       "type": bool, "default": False,
             "help": "Only affects the target ROOT group named above: when it doesn't exist, it is created before the import. It never creates per-row subgroups — rows whose group_path points at a missing subgroup still fall back per the placement precedence (see the fallback group's help)."},
            {"name": "dest_group",         "prompt": "Fallback group for rows whose path can't be matched under the target (blank = target root)", "type": str,  "widget": "group-picker", "optional": True,
             "help": "A per-row FALLBACK, not the import destination ('Target group' above is). Each row is placed by precedence:\n① its own group_path, when that exists under the target root (same-system re-import);\n② structural reconcile — strip the source root, match the relative path under the target root;\n③ this fallback group;\n④ the target root.\nThis field is only consulted at step ③ — i.e. only for rows whose structure can't be honored."},
            {"name": "source_root",        "prompt": "Source root to strip for cross-group placement (blank = use export stamp; rarely needed)", "type": str, "optional": True,
             "help": "For files exported from a DIFFERENT group hierarchy (e.g. another enclave), the row paths carry the source root. Stripping it lets each epic be re-placed into the structurally-matching group under this target root. Matching is by relative path — never by id or title.\n• Blank (default): use the 'source_root' stamp the export records (all exports are stamped since #139), or fall back to the paths' common prefix for older files.\n• Set: override the root to strip (a group full-path like 'ns-a/portfolio') when importing an un-stamped file or overriding a wrong stamp."},
            {"name": "create_missing_groups", "prompt": "Create missing subgroups along each row's reconciled path",           "type": bool, "default": False,
             "help": "When a row's structural path doesn't exist under the target root (incomplete mirror), create the missing subgroup chain (mkdir -p style) and place the epic there — instead of falling back to the fallback group / target root.\n• Applies only when the source root is TRUSTED (the export's stamp or an explicit Source root override) — a guessed root never creates groups, so a bad guess can't mint a wrong tree.\n• Created subgroups are named from the Group names file below when supplied; otherwise the path segment is used.\n• Dry run lists the groups that would be created without creating them."},
            {"name": "group_names",        "prompt": "Group names file from the export (optional — names created subgroups like the source)", "type": str, "widget": "file", "optional": True,
             "help": "Exports write a '<export>-group-names.json' sidecar mapping each subgroup/project path to its display name. Supplying it here makes create-missing-groups name recreated subgroups like the source (e.g. 'Value Stream 01' instead of the slug 'vs-01'). Without it, created containers use the path segment as their name."},
            {"name": "unresolved_parent",  "prompt": "Unresolvable parent_id action (ask / label / skip)",                     "type": str,  "widget": "select", "options": ["label", "skip"], "default": "label",
             "help": "What to do when a row's parent_id has no matching epic in the target hierarchy:\n• label (default): create the epic with no parent and add an 'import::needs-parent' label so you can find and re-parent it later.\n• skip: skip the affected rows entirely — nothing is created for them.\n• ask (CLI only): pause and pick one fallback parent from the live hierarchy for all affected rows; in the web UI this behaves as 'label'."},
            {"name": "on_existing",        "prompt": "If an epic with the same title exists (create / skip / update)",          "type": str,  "widget": "select", "options": ["create", "skip", "update"], "default": "skip"},
            {"name": "on_missing_bv_field", "prompt": "If the Business Value field is missing on the target (create / ignore / fail)", "type": str, "widget": "select", "options": ["create", "ignore", "fail"], "default": "create",
    ...
    },
    {
        ...
        "params": [
            {"name": "input_path",          "prompt": "Input file path (.csv or .json)",                            "type": str,  "widget": "file", "optional": False},
            {"name": "group",               "prompt": "Target group (import into)",                                 "type": str,  "widget": "group", "optional": True},
            {"name": "create_missing",      "prompt": "Create the target root group if it doesn't exist (subgroups/projects are NOT created)", "type": bool, "default": False,
             "help": "Only affects the target ROOT group named above: when it doesn't exist, it is created before the import. It never creates per-row subgroups or projects — rows whose project_path points at a missing project still fall back per the placement precedence (see the fallback project's help)."},
            {"name": "target_project_path", "prompt": "Fallback project for rows whose path can't be matched under the target (blank = skip such rows)", "type": str,  "widget": "project", "optional": True,
             "help": "A per-row FALLBACK, not the import destination ('Target group' above is). Each row is placed by precedence:\n① its own project_path, when that exists under the target root (same-system re-import);\n② structural reconcile — strip the source root, match the relative path under the target root;\n③ this fallback project;\n④ the row is SKIPPED — unlike epics, issues have no valid root container to fall back to.\nThis field is only consulted at step ③ — i.e. only for rows whose structure can't be honored."},
            {"name": "source_root",         "prompt": "Source root to strip for cross-group placement (blank = use export stamp; rarely needed)", "type": str, "optional": True,
             "help": "For files exported from a DIFFERENT group hierarchy (e.g. another enclave), the row paths carry the source root. Stripping it lets each issue be re-placed into the structurally-matching project under this target root. Matching is by relative path — never by id or title.\n• Blank (default): use the 'source_root' stamp the export records (all exports are stamped since #139), or fall back to the paths' common prefix for older files.\n• Set: override the root to strip (a group full-path like 'ns-a/portfolio') when importing an un-stamped file or overriding a wrong stamp."},
            {"name": "epic_id_map",         "prompt": "Epic id-map file from a prior epics import (optional — paired import)", "type": str, "widget": "file", "optional": True,
             "help": "A completed import-epics run writes an epic id map (source epic id → newly created id) to public/exports and prints its path. Supplying it here resolves each issue's epic_id to the epic just created on THIS system.\nResolution order per row: ① this id map; ② exact epic_title match among the target's epics (ambiguity warns, first wins); ③ the raw epic_id — same-root imports only, since an equal id on a different system would attach the issue to an unrelated epic. Unresolvable references warn and the link is dropped (the issue still imports)."},
            {"name": "create_missing_projects", "prompt": "Create missing projects (and their subgroups) along each row's reconciled path", "type": bool, "default": False,
             "help": "When a row's structural path doesn't exist under the target root (incomplete mirror), create the missing subgroup chain and the project (mkdir -p style) and place the issue there — instead of falling back / skipping.\n• Applies only when the source root is TRUSTED (the export's stamp or an explicit Source root override) — a guessed root never creates anything.\n• Created containers are named from the Group names file below when supplied; otherwise the path segment is used.\n• Dry run lists what would be created without creating it."},
            {"name": "group_names",         "prompt": "Group names file from the export (optional — names created containers like the source)", "type": str, "widget": "file", "optional": True,
             "help": "Exports write a '<export>-group-names.json' sidecar mapping each subgroup/project path to its display name. Supplying it here makes create-missing-projects name recreated subgroups and projects like the source. Without it, created containers use the path segment as their name."},
            {"name": "on_existing",         "prompt": "If an issue with the same title exists (create / skip / update)",  "type": str,  "widget": "select", "options": ["create", "skip", "update"], "default": "skip"},
            {"name": "dry_run",             "prompt": "Preview (dry run) — validate and preview only, nothing is created", "type": bool, "default": False, "cli_only": True},
        ],
    },
    {
        "key":         "export-links",
        "description": "Export all is_blocked_by relationships (epic←epic, issue←issue, epic←issue) to CSV or JSON",
        "method":      "export_links",
        "params": [
            {"name": "group",       "prompt": "Source group (export from)", "type": str, "widget": "group", "optional": True},
            {"name": "fmt",         "prompt": "Export format", "type": str, "widget": "select", "options": ["csv", "json"], "default": "csv"},
    ...
    {
        "key":         "import-links",
        "description": "Recreate exported is_blocked_by relationships on this system (run after importing epics/issues)",
        "method":      "import_links",
        "confirm":     True,
        "params": [
            {"name": "input_path",  "prompt": "Links file path (.csv or .json)", "type": str, "widget": "file", "optional": False},
            {"name": "group",       "prompt": "Target group (import into)", "type": str, "widget": "group", "optional": True},
            {"name": "epic_id_map", "prompt": "Epic id-map file from a prior epics import (optional — paired import)", "type": str, "widget": "file", "optional": True,
    ...
    },
    {
        ...
        "params": [
            ...
            {"name": "limit",  "prompt": "Max results (blank = 100, 'all' = uncapped)", "type": str, "optional": True,
             "help": "A positive integer caps the returned window; 'all' fetches every match (no cap)."},
