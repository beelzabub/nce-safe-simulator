...
TOOLS = [
    ...
    },
    {
        ...
        "params": [
            ...
            {"name": "dest_group",         "prompt": "Destination group for epics whose group_path doesn't resolve (blank = target root)", "type": str,  "widget": "group-picker", "optional": True},
            {"name": "source_root",        "prompt": "Source root to strip for cross-group placement (blank = use export stamp)", "type": str, "optional": True,
             "help": "For files exported from a DIFFERENT group hierarchy (e.g. another enclave), the row paths carry the source root. Stripping it lets each epic be re-placed into the structurally-matching group under this target root.\n• Blank (default): use the 'source_root' stamp the export records, or fall back to the paths' common prefix.\n• Set: override the root to strip (a group full-path like 'ns-a/portfolio') when importing an un-stamped file."},
    ...
    },
    {
        ...
        "params": [
            ...
            {"name": "group",               "prompt": "Target group (import into)",                                 "type": str,  "widget": "group", "optional": True},
            {"name": "create_missing",      "prompt": "Create target group if it doesn't exist",                    "type": bool, "default": False},
            {"name": "target_project_path", "prompt": "Destination project for issues whose project_path doesn't resolve (blank = use project_path column)", "type": str,  "widget": "project", "optional": True},
            {"name": "source_root",         "prompt": "Source root to strip for cross-group placement (blank = use export stamp)", "type": str, "optional": True,
             "help": "For files exported from a DIFFERENT group hierarchy (e.g. another enclave), the row paths carry the source root. Stripping it lets each issue be re-placed into the structurally-matching project under this target root.\n• Blank (default): use the 'source_root' stamp the export records, or fall back to the paths' common prefix.\n• Set: override the root to strip (a group full-path like 'ns-a/portfolio') when importing an un-stamped file."},
