...
class ReportsMixin:
    ...
    def generate_workload_report(self):
        ...
        md.extend([
            ...
            "- **🧩 Capability** — a Large Solution-level deliverable decomposed from an Epic; sized to fit within a PI across one or more ARTs",
            "- **🛠️ Feature** — a service or function delivered by a single ART within one PI; directly enables business or technical outcomes",
            "- **Planned**: sum of epic planned weights committed to this PI in this group",
            "- **Actual**: sum of issue weights for those Features (team-entered estimate)",
            "- **Δ**: Actual − Planned (▲ more work than planned, ▼ less, = matched)",
            "- **% Done**: weighted average completion across Features (by planned weight)",
            "- **Status**: ✅ On Track if % Done ≥ % elapsed through PI · ⚠️ At Risk if behind · ❌ Incomplete if PI ended with work remaining",
            "",
        ])
