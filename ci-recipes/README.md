...
| [`containerize-bootstrap.yml`](containerize-bootstrap.yml) | **First-run image build for an empty registry.** The baseline `containerize` build/push without its `test` gate — required once after a fresh import (e.g. an enclave lift without the image phase), because `test` runs in the dev image (#279) and the gated `containerize` can never produce it while `test` is red. Run from the default branch; not for routine builds. |
...
| [`security-evidence.yml`](security-evidence.yml) | **Is the committed evidence still current?** Regenerates `docs/security/{EVIDENCE.md,dispositions.csv,evidence.json}` from the live Vulnerability Report, publishes them as artifacts, and prints what moved against the committed copy. Commits nothing, and is **green whether or not there is drift** — drift is information, and the commit stays a deliberate human review step (#317). Needs a `read_api` token; `CI_JOB_TOKEN` cannot read vulnerabilities. Worth a schedule. |
