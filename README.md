...
| Area | Content |
|------|---------|
| Top nav | PMW 120 / NCE Safe Simulator wordmark; ⌕ Search (opens the JQL search view at `/search`); running-job count badge; dark ↔ light theme toggle; Status panel toggle |
...
The **Search** button in the nav bar opens `/search` (epic #297, issue #302) — the "issue navigator" GitLab's work-items search bar lacks. Type a JQL expression, get the matching work items back **live** (every query runs against the GitLab API — connectivity and a token required; results are always current, no snapshot involved). The language is real JQL syntax over the simulator's field vocabulary: `AND` / `OR` / `NOT`, parentheses, `= != > >= < <= ~ !~`, `IN` / `NOT IN`, `IS [NOT] EMPTY`, relative dates (`-4w`), date functions (`now()`, `startOfDay()`, `endOfWeek()`…), `currentUser()`, and multi-key `ORDER BY`. Fields cover the core work-item attributes (`type`, `state`, `title`, `labels`, `assignee`, `author`, `milestone`, `iteration`/`sprint`, `weight`, `parent`, `iid`, `created`/`updated`/`due`/`closed` dates), the scoped-label virtual fields generated from every `*_labels` taxonomy in `config.json` (`piid`, `epic_type`, `risk`, `wsjf_urgency`, …), `business_value`, plus Jira aliases for the obvious names (`status`→`state`, `issuetype`→`type`, `sprint`→`iteration`, `summary`→`title`). Example queries:

```
state = opened AND weight >= 5 ORDER BY due ASC
type = epic AND labels = "epic::feature" AND updated >= -4w
state = opened AND (assignee = currentUser() OR assignee IS EMPTY) AND piid = 2026Q2
business_value >= 8 ORDER BY business_value DESC, weight DESC
```

The view is a query box (with clickable example queries in the empty state), a `limit` control, and a results table showing IID, linked title (opens the GitLab work item), project, type, state, labels, weight, assignees, and the created/updated/due dates. **Label chips honor the colors set in GitLab** — the result envelope carries a `label_colors` map (`title -> {color, text_color}`, harvested from the same GraphQL nodes as the results, so no extra request) and the chips use GitLab's own text color, falling back to the YIQ contrast rule epic-cards uses; labels never styled in GitLab keep the default chip look. A **⚙ Columns** button in the results bar opens a checkbox popover over the full flat item schema (author, milestone + due, iteration, business value, start/closed dates, parent IID, namespace, …; project is the derived namespace leaf — what the JQL `project` predicate matches — while Namespace keeps the full path) — the default set is the eleven columns above, and **◀ ▶ arrows reorder the visible columns** (visible columns list first, in table order; newly ticked ones append at the end). At least one column always stays, the set *and* order persist per browser (localStorage, like the rest of the dialog state from #80), and Reset restores the default set and order. Hidden columns still export: CSV always carries the full schema. The header names the **exact group slug being queried** (from `/api/config`'s `target_group_path`) so the scope is never ambiguous, and the far right of the query bar carries two Jira-style affordances: an **expand toggle** that swaps the input for a multi-line editor (newlines are whitespace to the parser; Ctrl/Cmd-Enter runs), and a **"?" help panel** laying out the operator/date syntax, the full field vocabulary — fetched live from `GET /api/query/fields`, so taxonomy fields list the exact values valid in *this* config — and an indexed set of example queries **assembled from those same live values** (e.g. your real `piid` quarters), each click-to-run and copyable for the CLI or API. **Clicking a column header re-sorts the displayed rows client-side** (toggle asc/desc, empties last) — this is a re-sort of the *fetched* result set only; the query's `ORDER BY` still defines the fetch order, and a "reset to query order" link undoes it. When the result set was capped by `limit`, a **"sorted locally — first N results"** hint appears next to the truncation note so a header sort isn't mistaken for a true top-N by that column. When more matches exist beyond the window, the results bar shows a **‹ Prev / Next ›** pager, and the **exact match count** whenever the engine knows it (see `total` below): the summary reads `100 of 342 results` and the range display `101–200 of 342`; only when the total is genuinely unknowable without a full scan (an early-stopped query with a client-side filter) does it fall back to `capped at limit N — total unknown`. Each page re-runs the query with a shifted `offset`, and a new Run resets to the first window. The results bar also carries **⬇ CSV / ⬇ JSON export buttons**. **CSV is the data-extract path and always covers *every* match** — on a result-limited page it first re-runs the query with `limit: "all"` (no window), then downloads the full set: CLI-`csv`-format columns (flat schema, list cells joined with `', '`), in the displayed order with any active header sort applied to the whole set. JSON stays the untouched `POST /api/query` envelope for the current page (items in fetch order plus `count`/`total`/`truncated`/`plan`). Parse errors render inline: the offending query echoes in monospace with a caret anchored at the reported character position, plus the parser's list of what was expected there; unknown fields/values come back with the full valid vocabulary spelled out.

Underneath sits `POST /api/query` — one endpoint, executing through the **same `run_jql()` entry point as the CLI query tool**, so the two surfaces return identical rows for identical queries. Body is `{"jql": "<query>", "limit": <positive int or "all">, "offset": <optional int ≥ 0>}` (limit defaults to 100 and `"all"` removes the cap entirely — the executor then fetches every match; offset defaults to 0 — offset/limit page through one stable result sequence, and the envelope echoes `offset`); the response envelope carries `items` (flat dicts: `iid`, `type`, `title`, `state`, `labels`, `assignees`, `author`, `milestone`, `iteration`, `weight`, `business_value`, dates, `parent_iid`, `namespace_path`, `web_url`), `count`, `limit`, `truncated` (true when the limit cut the list or pages were left unfetched), `label_colors` (title → `{color, text_color}` for matched items' labels, from GitLab's label definitions), `total` — the **exact number of matches across the whole result set**, known whenever the scan reached the last page (any untruncated query, `limit: "all"`, or a client-side sort, which always fetches everything) *or* every predicate was pushed down to GitLab (the GraphQL connection's own `count` is then exact even on a truncated first page); `null` only when an early-stopped scan left a client-side filter unevaluated on unfetched pages — and a `plan` block showing what was pushed down to GitLab as filter args vs evaluated client-side:

```bash
curl -s -X POST http://localhost:4645/api/query \
  -H 'Content-Type: application/json' \
  -d '{"jql": "state = opened AND weight >= 5 ORDER BY due ASC", "limit": 50}'
```

`GET /api/query/fields` returns the queryable vocabulary — `{"fields": [{"name", "kind" (core/label/custom), "type", "aliases", "values"}]}` — from the same registry `run_jql` validates against, so it always reflects the live config (the help panel is its consumer).

Bad queries never 500. Syntax errors return **400** with a structured detail — `{"kind": "syntax", "message", "position", "found", "expected": [...]}` — which is exactly what the view anchors the inline caret with; semantic errors (unknown field, out-of-vocabulary taxonomy value, invalid operator/field combination) return **400** with `{"kind": "semantic", "message"}`; a malformed request body (missing `jql`, a `limit` that isn't a positive integer or `"all"`) is **400** `{"kind": "request"}`; GitLab transport failures are **502**; a server started without a GitLab connection answers **503**.
...
|---|---|---|---|
| `test` | build | every push | Runs the suite **in the pipeline's own dev image** (`$CI_REGISTRY_IMAGE/dev:latest`, issue #279) — toolchain, Pango/fonts, and requirements pre-baked, zero at-job package fetches — then `pip install -r requirements.txt` (delta-catcher for branch-added deps) `&& pytest tests/` |
...
`containerize` gates on `test` (`needs: [test]`), so an image is never published from a failing suite. Because `test` itself runs in the dev image `containerize` publishes (#279), an **empty registry** (fresh import, new space) deadlocks the baseline pipeline — bootstrap it by running the [`containerize-bootstrap` recipe](ci-recipes/README.md) once from the default branch; every later pipeline then works unaided. The dev image `test` pulls is the one from the *previous* default-branch merge — the retained `pip install` covers Python deps added on a branch, while Dockerfile-level (system) changes only take effect after merging. It uses the built-in `$CI_JOB_TOKEN` to authenticate to the registry — no secret to configure. The explicit-request path lets an MR branch prove the full image build *before* it gates the default branch (used to audit issue #269's registry-only closure from the kaniko log); ordinary pushes stay test-only, and branch-built tags are reclaimed by the next default-branch pipeline. See [Container-Based Development & Registry](#container-based-development--registry) for how developers consume the published images. The job also runs on runners configured with Docker init injection (`FF_USE_INIT_WITH_DOCKER_EXECUTOR`, which bind-mounts a supervisor at `/sbin/docker-init` and otherwise breaks kaniko's `/sbin` replacement on Debian usrmerge bases with "device or resource busy" — issue #276, upstream kaniko#946): the job sets that feature flag to `"false"` and passes `--ignore-path=/sbin` to both kaniko invocations, so either the mount never appears or kaniko skips the `/sbin` swap. Both guards are inert on runners without init injection, like gitlab.com's, and the pushed image keeps the base's `/sbin` symlink since kaniko ships base layers unmodified. Last resort on a runner that pins the flag on in its own `config.toml`: the runner admin removes it from `[runners.feature_flags]`.
...
##### CI system packages (`weasyprint-apt-debs`) — retired

Historical (issues #261 → #279): the `test` job needs Pango and a font at the system level (WeasyPrint renders a real PDF in the suite; pip can't supply these), and it used to fetch three `.deb`s from generic package `weasyprint-apt-debs` (version `2026.07.21`) at job time and `dpkg -i` them onto stock `python:3.11`. Since issue #279 the job runs in the pipeline's own **dev image**, which inherits the WeasyPrint closure from the runtime stage's [`apt-debs`](#image-build-system-packages-apt-debs) install (issue #269) — so the package has **no consumer left**. It stays in the registry (existing enclave exports enumerate packages live and carry it harmlessly), but there is nothing to refresh anymore; the closure is maintained solely in `apt-debs`.
...
**Fields.** Core work-item fields: `type` (`epic` / `issue`), `state` (`opened` / `closed` / `all`), `title`, `text` (title + description), `labels`, `assignee`, `author`, `milestone`, `iteration`, `weight`, `business_value` (native custom field), `created`, `updated`, `due`, `closed`, `parent`, `iid`, `project`. Plus one **virtual field per `*_labels` taxonomy in `config.json`**: `piid`, `epic_type`, `risk`, `work_type`, `lifecycle`, `wsjf_urgency`, `wsjf_risk`, `project_label`, … — `piid = 2026Q3` matches the `PIID::2026Q3` scoped label (bare value or full label, case-insensitive). Jira names alias to the obvious equivalents: `status` → `state`, `issuetype` → `type`, `summary` → `title`, `reporter` → `author`, `sprint` → `iteration`. People fields (`assignee`, `author`/`reporter`, `currentUser()`) match the **username or the display name**, case-insensitively — `assignee = beelzabub` and `assignee = "Jamie Powers"` return the same rows (people equality therefore evaluates client-side; `assignee IS EMPTY` still pushes down).

**Params:** `--jql` (the query text, **or a path to a file containing it** — a value naming an existing file is read UTF-8, multi-line queries welcome; a "Query read from <path>" note goes to stderr), `--limit` (result cap, default 100; **`all` removes the cap** and fetches every match), `--offset` (skip that many matches first — `--limit`/`--offset` page through one stable result sequence; truncation notes name the next offset), `--format` (`table` default / `json` / `csv`).
...
# CSV export of EVERY match — --limit all removes the result cap
python3 NceGitLab.py -ut query --jql 'labels IN ("PIID::2026Q3", "PIID::2026Q4")' --limit all --format csv > pi-items.csv
...
`json` / `csv` print the payload alone on **stdout** (the runner's banner and the connection diagnostics go to stderr), so shell pipes see clean machine-readable output. The `json` envelope carries `items`, `count`, `limit`, `offset`, `total` (the exact whole-set match count whenever it is knowable — see the `/api/query` docs above; `null` otherwise), `truncated`, and a `plan` block showing which filters were pushed down server-side and what ran client-side. When a result is cut at the limit, `table`/`csv` print a truncation note (`csv`'s goes to stderr) that names the exact total when known (`Truncated at limit 100 of 342 total matches …`) and always names the next `--offset` and the `--limit all` escape hatch.
...
When run from a non-interactive shell (no TTY — e.g. a CI job), the tool never prompts: any option you don't pass takes its default, so `python3 NceGitLab.py -ut epic-cards --output_path deck.pdf` runs unattended (group from `config.json`, filter + taxonomy from the spec). The deck ships as a runnable CI recipe: start a pipeline with variable `RECIPE=epic-cards-deck` (see [Running in a CI/CD pipeline](#running-in-a-cicd-pipeline)) and the PDF publishes as a pipeline artifact — the default pipeline stays `test` + `containerize` only (issue #275); recipe runs never touch it. Prerequisite: the `GITLAB_API_TOKEN` CI/CD variable (masked, `api`/`read_api` scope) — and mind the **Protected** flag: a protected variable is only injected into pipelines on protected refs, so on an instance with no protected branches it is injected nowhere.
...
The top-level `.gitlab-ci.yml` is a **router** (issue #283). With no variable it runs the baseline pipeline — `test` + `containerize`, issue #275's ground zero — exactly as always on every push and merge. Start a pipeline with a `RECIPE=<name>` variable instead (CI/CD → Pipelines → **Run pipeline**, or `glab ci run -b <branch> --variables RECIPE:<name>`, or a pipeline schedule that defines `RECIPE`) and the baseline jobs sit out while [`ci-recipes/<name>.yml`](ci-recipes/) runs as a **child pipeline** — nothing is ever copied into or restored over `.gitlab-ci.yml`. Extra variables on the same run are forwarded to the child, so recipes can take parameters. Each recipe is a self-contained child pipeline (own `stages:`, own `image:`); its header documents prerequisites (CI/CD variables, access tokens), and secrets go in masked CI/CD variables, never in the file. The recipe list and authoring guide live in [`ci-recipes/README.md`](ci-recipes/README.md); shipping today: [`smoke.yml`](ci-recipes/smoke.yml) (router plumbing check and authoring skeleton), [`epic-cards-deck.yml`](ci-recipes/epic-cards-deck.yml) (Capability Card PDF as a downloadable artifact), [`all-reports.yml`](ci-recipes/all-reports.yml) (full report suite inside the project's own runtime image — Quarto, Pango, and every pip dependency baked in, no external downloads at job time — publishing the rendered `public/` site as the artifact), [`kaniko-runner-diag.yml`](ci-recipes/kaniko-runner-diag.yml) (the issue #276 runner diagnostic, now one click instead of a yaml swap), and six security-scanning recipes (`security-sast`, `security-secret-detection`, `security-dependency-scanning`, `security-container-scanning`, `security-iac`, and the one-shot `security-all` sweep) wrapping GitLab's maintained scanner templates — findings surface in the pipeline Security tab and (on default-branch runs) the Vulnerability report; the project wiki's **Security scanning** pages walk each scanner's UI. Alongside them, [`security-evidence.yml`](ci-recipes/security-evidence.yml) (issue #317) answers whether the committed compliance evidence in [`docs/security/`](docs/security/) still describes reality: it regenerates the disposition register from the live Vulnerability Report, publishes it as an artifact, and prints what moved — findings added, no longer reported, or with a changed disposition — **committing nothing and staying green whether or not there is drift**. Refreshing the committed copy stays a deliberate `scripts/security_evidence.py` run followed by a human commit, because the register asserts that a person reviewed those findings and accepted the risk; a job that re-stamped that approval on every push would make the artifact worth less as evidence, not more. It needs a masked `read_api` token (`SECURITY_EVIDENCE_TOKEN`, `GITLAB_API_TOKEN` or `GITLAB_TOKEN`) — `CI_JOB_TOKEN` cannot read project vulnerabilities — and is a natural fit for a pipeline schedule.
...
- Reports: the [`all-reports.yml`](ci-recipes/all-reports.yml) recipe (run a pipeline with variable `RECIPE=all-reports`) runs entirely from the imported runtime image — no external downloads at job time — and is the end-to-end proof that report generation works inside the enclave.
