...
The **Blocked Work Explorer** (Analysis tab) renders this: a totals strip (portfolio epics at risk · blocked items · blocked weight · BV at risk), then one card per threatened portfolio epic — state, linked title, PIID/project chips, progress, and blocked-weight / BV-at-risk badges — sorted by BV at risk so the biggest fires come first. Cards expand to the hierarchy chains (🏆 Epic → 🧩 Capability → 🛠️ Feature) with the blocked node flagged, its blockers linked, and per-node weight/BV figures; the top card starts expanded. Empty states cover no-snapshot (with a call-to-action to run reports) and all-clear.
