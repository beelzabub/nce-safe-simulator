...
When run from a non-interactive shell (no TTY — e.g. a CI job), the tool never prompts: any option you don't pass takes its default, so `python3 NceGitLab.py -ut epic-cards --output_path deck.pdf` runs unattended (group from `config.json`, filter + taxonomy from the spec). See `ci-recipes/` for a ready-to-copy GitLab CI job that publishes the deck as a pipeline artifact — the shipped `.gitlab-ci.yml` deliberately stays `test` + `containerize` only (issue #275); copy the recipe in when a space wants the deck job, and restore ground zero anytime with `cp ci-recipes/baseline-gitlab-ci.yml .gitlab-ci.yml`. Prerequisite when you do: the `GITLAB_API_TOKEN` CI/CD variable (masked, `api`/`read_api` scope) — and mind the **Protected** flag: a protected variable is only injected into pipelines on protected refs, so on an instance with no protected branches it is injected nowhere.
