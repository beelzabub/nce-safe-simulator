...
This repo's own pipeline also carries the job (issue #274): **`epic-cards-deck`** appears as a **manual** job on every branch pipeline — press ▶ to render `capability-cards.pdf` as a 30-day artifact. It runs in the project's runtime image, so nothing is installed at job time (no apt, no PyPI tree — the same zero-egress contract as the image builds, and why the button keeps working on a lifted enclave repo once images are imported). Prerequisite: the `GITLAB_API_TOKEN` CI/CD variable (masked, `api`/`read_api` scope). Mind the **Protected** flag: a protected variable is only injected into pipelines on protected refs — on an instance with no protected branches it is injected nowhere and the job stops with an explanatory error.
