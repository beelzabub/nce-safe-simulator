...
# Findings panel is driven by the committed evidence file (issue #304, Section 2)
# — docs/security/evidence.json, regenerated by scripts/security_evidence.py after
# a scan — so the deck never drifts from the real register (retires the old
# hardcoded counts, #292 item 2).
HERE = os.path.dirname(os.path.abspath(__file__))
EVIDENCE_PATH = os.path.join(os.path.dirname(HERE), "docs", "security", "evidence.json")

_SCANNER_META = {
    "CONTAINER_SCANNING": ("Container Scanning", "Trivy — published runtime image"),
    "DEPENDENCY_SCANNING": ("Dependency Scanning", "requirements.txt + frontend lockfiles"),
    "SAST": ("SAST / IaC", "Semgrep + KICS — source and deploy code"),
    "SECRET_DETECTION": ("Secret Detection", "Gitleaks — commit history"),
...
def _scanners_from_evidence(ev):
    ver = {}
    for s in ev.get("scanners", []):
        t = (s.get("type") or "").upper().replace("-", "_")
        if s.get("scanner") and s.get("version"):
            ver[t] = f"{s['scanner']} {s['version']}"
    rows = []
    for rt, c in ev["by_scanner"].items():
        label, sub = _SCANNER_META.get(rt, (rt, ""))
        sub = ver.get(rt, sub)
        sevs = [(k, v) for k, v in (c.get("detected_by_severity") or {}).items()]
        note = f"{c['dismissed']} dismissed · {c['resolved']} resolved — see docs/security/EVIDENCE.md"
        rows.append((label, sub, c["detected"], sevs, note))
    return rows


EVIDENCE = _load_evidence()
SCANNERS = _scanners_from_evidence(EVIDENCE)
...
def render_findings(out_path):
    ...
    d.text((x0 + 40, 88), f"Security posture — develop @ {EVIDENCE['scanned_sha']}", font=F_TITLE(34), fill=INK)
    d.text((x0 + 40, 150),
           f"{EVIDENCE['generated_at'][:10]} · pipeline #{EVIDENCE['scan_pipeline']['id']} · "
           f"every finding fixed or dispositioned",
           font=F_BODY(24), fill=GRAY)
    total = EVIDENCE["totals"]["detected"]
    tf = F_TITLE(52)
    ttext = f"{total} open"
    d.text((x1 - 60 - d.textlength(ttext, font=tf), 100), ttext, font=tf,
           fill=(GREEN if total == 0 else BLUE))
